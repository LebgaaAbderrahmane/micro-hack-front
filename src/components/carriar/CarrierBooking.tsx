...
function generateTimeSlotsForDay(date: Date): TimeSlot[] {
  ...
  while (i < HOURS.length) {
    ...
    const durationHours = i < HOURS.length - 1 && Math.random() > 0.8 ? 2 : 1;
...
export default function CarrierBookingPage() {
  ...
  const [terminal, setTerminal] = useState<"terminal-a" | "terminal-b">(
    "terminal-a",
  );
  ...
  const filteredBookings = useMemo(() => {
    return bookings.filter((b) => {
      ...
      const typeMatch =
        typeFilter.length === 0 || typeFilter.includes(b.containerType);
      const dateMatch =
  ...
  const handleBulkDownload = () => {
    const selected = bookings.filter(
      (b) => selectedBookings.has(b.id) && b.status === "confirmed",
    );
  ...
  const handleBulkCancel = () => {
    ...
    setBookings((prev) =>
      prev.map((b) =>
        selectedIds.includes(b.id) && b.status === "confirmed"
          ? { ...b, status: "cancelled" }
          : b,
      ),
    );
      ...
      <div className="min-h-screen bg-slate-50 p-6 space-y-6">
        ...
        <Card>
          <CardHeader>
            <div className="flex justify-between items-center">
              ...
              <div className="flex gap-4 text-sm">
                <span className="flex items-center gap-2">
                  <div className="w-3 h-3 bg-green-500 rounded" /> Available
                </span>
                <span className="flex items-center gap-2">
                  <div className="w-3 h-3 bg-orange-500 rounded" /> Limited
                </span>
                <span className="flex items-center gap-2">
            ...
            <CardDescription>
              Scroll horizontally to see all hours (06:00 - 23:00)
            </CardDescription>
          </CardHeader>
          <CardContent>
            <ScrollArea className="w-full border rounded-md">
              <div className="min-w-[1600px]">
                ...
                >
                  {/* Header Row */}
                  <div className="p-3 border-b border-r bg-slate-50 font-semibold text-center sticky left-0 z-10">
                    Day
                  </div>
                  {HOURS.map((h) => (
                    <div
                      key={h}
                      className="p-3 border-b border-r text-center text-sm font-medium bg-slate-50"
                    >
                          ...
                          <Tooltip key={slot.id}>
                            <TooltipTrigger asChild>
                              <div
                                ...
                                className={`h-16 border-b border-r p-1 transition-all ${
                                  slot.status === "full"
                              ...
                              >
                                <div
                                  className={`h-full w-full rounded-md ${getSlotColor(slot.status)} flex flex-col items-center justify-center text-white text-xs shadow-sm`}
                                >
                                  <span className="font-bold">
                ...
                {filteredBookings.length === 0 ? (
                  ...
                  filteredBookings.map((booking) => (
                    <TableRow key={booking.id}>
                      ...
                      <TableCell>
                        <Badge variant="outline">{booking.containerType}</Badge>
                      </TableCell>
        ...
        >
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                {selectedSlot &&
                  format(selectedSlot.startTime, "EEEE, MMMM d 'at' HH:mm")}
              </DialogDescription>
            </DialogHeader>
