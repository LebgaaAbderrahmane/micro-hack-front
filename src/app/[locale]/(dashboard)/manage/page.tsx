...
export default function ManagePage() {
    ...
    const [activeTab, setActiveTab] = useState<"users" | "terminals" | "gates">(
        "users",
    );
    ...
    const fetchTabData = async (tab: string) => {
        ...
        try {
            if (tab === "users") {
                const usersRes = await usersService.getWithAuth();
                ...
                const res = await gatesService.getAll();
                const rawData = res.data || [];
                setGates(
                    rawData.map((g: any) => ({
                        id: g.id,
                        ...
                        lastSync: g.modified_at
                            ? new Date(g.modified_at).toLocaleTimeString()
                            : "Never",
                    })),
                );
            }
    ...
    const handleToggleAll = (type: "users" | "terminals" | "gates") => {
        if (type === "users") {
            ...
            setSelectedTerminals(
                selectedTerminals.length === filteredTerminals.length
                    ? []
                    : filteredTerminals.map((t) => t.id),
            );
        } else if (type === "gates") {
            setSelectedGates(
                selectedGates.length === filteredGates.length
                    ? []
                    : filteredGates.map((g) => g.id),
            );
        }
    };
    ...
    const handleSaveEdit = async () => {
        ...
        try {
            if (activeTab === "users") {
                ...
                await terminalsService.update(editingItem.id, {
                    zone_name: editingItem.name,
                    zone_code: editingItem.detail,
                });
            } else if (activeTab === "gates") {
                await gatesService.update(editingItem.id, {
                    gate_number: editingItem.name,
                    gate_status: editingItem.detail as GateStatus,
                });
            }
        ...
        try {
            if (activeTab === "users") {
                ...
                await terminalsService.create({
                    ...
                    zone_code: newItemDetail || "T-01",
                    total_capacity: 100,
                    port_id: "PORT_001",
                });
                toast.success("Terminal added");
            } else if (activeTab === "gates") {
                await gatesService.create({
                    gate_number: newItemName,
                    physical_capacity: parseInt(newItemDetail) || 10,
                    port_id: "PORT_001",
    ...
    const handleToggleGateStatus = async (gate: GateData) => {
        const newStatus: GateStatus =
            gate.status === "OPERATIONAL" ? "CLOSED" : "OPERATIONAL";
        try {
    ...
    const RoleBadge = ({ role }: { role: UserRole }) => {
        ...
        return (
            <Badge
                className={cn(
                    "text-white font-medium text-xs px-2.5 py-0.5 border-none shadow-none font-poppins",
                    roles[role],
                )}
            >
                {role}
    ...
    return (
        <div className="flex flex-col gap-6 w-full max-w-[1400px] mx-auto py-8 px-6 min-h-screen relative">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-2 px-1">
                <div className="space-y-1">
                    ...
                    <p className="text-slate-500 dark:text-slate-400 text-sm font-medium">
                        Configure network entities, user permissions and gate protocols
                    </p>
                </div>
                <div className="flex items-center gap-3">
                    <Button
                        ...
                        className="px-6 h-11 gap-2 font-poppins font-bold text-xs rounded-xl shadow-sm hover:bg-foreground/5"
                    >
                        Export
                    </Button>
                    <Button
                    ...
                    <Table>
                            ...
                            <TableRow className="hover:bg-transparent h-12">
                                <TableHead className="w-[60px] text-center">
                                    <Checkbox
                                        ...
                                        onCheckedChange={() => handleToggleAll(activeTab)}
                                ...
                                {activeTab === "users" && (
                                    <>
                                        <TableHead className="font-bold text-slate-800 dark:text-slate-200 text-[11px] uppercase tracking-wider">
                                            User Identity
                                        </TableHead>
                                        <TableHead className="font-bold text-slate-800 dark:text-slate-200 text-[11px] uppercase tracking-wider">
                                            Email Address
                                        </TableHead>
                                        <TableHead className="font-bold text-slate-800 dark:text-slate-200 text-[11px] uppercase tracking-wider text-center">
                                            Role
                                        </TableHead>
                                        <TableHead className="font-bold text-slate-800 dark:text-slate-200 text-[11px] uppercase tracking-wider text-center">
                                            Status
                                ...
                                <TableHead className="font-bold text-slate-800 dark:text-slate-200 text-[11px] uppercase tracking-wider text-right pr-6">
                                    Management
                                </TableHead>
                            </TableRow>
                        </TableHeader>
                        <TableBody>
                            {isLoading && (
                                <TableRow>
                                    <TableCell
                                        colSpan={activeTab === "users" ? 6 : 6}
                                        className="text-center py-20 text-slate-500 font-poppins"
                                    >
                            ...
                            {!isLoading && activeTab === "users" &&
                                filteredUsers.map((user) => (
                                    <TableRow
                                        key={user.id}
                                        className="group hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition-colors border-slate-100 dark:border-slate-800/60 h-16"
                                    >
                                        <TableCell className="text-center font-poppins">
                                            <Checkbox
                                                checked={selectedUsers.includes(user.id)}
                                                onCheckedChange={() =>
                                                    handleToggleSelection(user.id, "users")
                                                }
                                                className="rounded-md border-slate-300"
                                            />
                                        </TableCell>
                                        <TableCell className="font-bold text-slate-700 dark:text-slate-200 font-poppins text-sm">
                                            {user.name}
                                        </TableCell>
                                        <TableCell className="text-slate-500 dark:text-slate-400 font-medium font-poppins text-sm">
                                            {user.email}
                                        </TableCell>
                                        ...
                                        <TableCell className="text-center text-slate-500 dark:text-slate-400 text-xs font-medium font-poppins tracking-tight">
                                            {user.lastActive}
                                        </TableCell>
                                        <TableCell className="text-right pr-6">
                                            <div className="flex items-center justify-end gap-1">
                                                <Button
                                                    ...
                                                    className="h-9 w-9 text-slate-400 hover:bg-tile-blue/10 hover:text-tile-blue transition-colors rounded-lg"
                                                >
                                                    <Pencil size={16} />
                                                </Button>
                                                <Button
                                                    ...
                                                    className="h-9 w-9 text-slate-400 hover:bg-error/10 hover:text-error transition-colors rounded-lg"
                                                >
                                                    <Trash2 size={16} />
                                                </Button>
                                            </div>
                                    ...
                                    >
                                        <TableCell className="text-center">
                                            <Checkbox
                                                ...
                                                onCheckedChange={() =>
                                                    handleToggleSelection(terminal.id, "terminals")
                                                }
                                                className="rounded-md border-slate-300"
                                            />
                                        </TableCell>
                                        ...
                                        <TableCell className="text-slate-500 dark:text-slate-400 font-bold text-xs uppercase tracking-widest bg-slate-100 dark:bg-slate-800/50 px-2 py-1 rounded inline-block mt-4 ml-4">
                                            {terminal.location}
                                        </TableCell>
                                        <TableCell className="text-center font-bold text-slate-700 dark:text-slate-300 font-poppins">
                                            {terminal.capacity}
                                        </TableCell>
                                        <TableCell>
                                            <ProgressBar
                                    ...
                                    >
                                        <TableCell className="text-center">
                                            <Checkbox
                                                ...
                                                onCheckedChange={() =>
                                                    handleToggleSelection(gate.id, "gates")
                                                }
                                                className="rounded-md border-slate-300"
                                            />
                                        </TableCell>
                                        ...
                                        <TableCell className="text-center text-slate-500 dark:text-slate-400 text-[10px] font-bold font-poppins tracking-tighter uppercase">
                                            {gate.lastSync}
                                        </TableCell>
                                        <TableCell className="text-right pr-6">
                                            <div className="flex items-center justify-end gap-1">
                            ...
                            {((activeTab === "users" && filteredUsers.length === 0) ||
                                ...
                                (activeTab === "gates" && filteredGates.length === 0)) && (
                                    <TableRow>
                                        <TableCell
                                            colSpan={7}
                                            className="h-32 text-center text-slate-400 font-poppins italic"
                                        >
            ...
            <Dialog open={isAddOpen} onOpenChange={setIsAddOpen}>
                <DialogContent className="sm:max-w-[420px] rounded-3xl p-8 gap-6 border-slate-200 dark:border-slate-800">
                    <DialogHeader>
                        <DialogTitle className="text-2xl font-bold font-poppins tracking-tight">
                            Register{" "}
                            {activeTab === "users"
                                ? "Operator"
                                : activeTab === "terminals"
                    ...
                    <DialogFooter>
                        <Button
                            ...
                            className="rounded-full font-poppins font-bold text-xs px-6 h-10"
                        >
                            Cancel
                        </Button>
                        <Button
                            onClick={handleAdd}
                            className="bg-tile-blue text-white rounded-full font-poppins font-bold text-xs px-6 h-10"
                        >
                            Create Entry
                        </Button>
                    </DialogFooter>
                        ...
                        <div className="space-y-4 py-2">
                            ...
                            <div className="space-y-2">
                                ...
                                <Input
                                    ...
                                    onChange={(e) =>
                                        setEditingItem({ ...editingItem, detail: e.target.value })
                                    }
                                    placeholder="Details..."
                                    className="rounded-lg border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 h-10 font-poppins placeholder:text-slate-400 dark:placeholder:text-slate-500 text-foreground"
                                />
            ...
            <ExportModal
                ...
                data={activeTab === "users" ? filteredUsers : activeTab === "terminals" ? filteredTerminals : filteredGates}
                type={activeTab === "users" ? "logs" : "bookings"} // ExportModal handles arrays in generic mode if type is not specifically fleet
