    ...
    <tr className="group hover:bg-foreground/[0.02] transition-colors border-b border-foreground/5 last:border-0 text-foreground">
      <td className="py-5 px-8">
        ...
        >
          ...
          <div>
            <div className="font-bold group-hover/link:text-primary transition-colors">
        ...
        <div
          className={cn(
            ...
            roleColors[user.role] ||
              "text-foreground bg-foreground/10 border-foreground/5",
          )}
        >
          {roleIcons[user.role]}
          {user.role}
      ...
      <td className="py-5 px-8 text-right">
        <Link
          href={`/users/detail?id=${user.id}`}
          className="p-2 hover:bg-foreground/5 rounded-lg text-foreground/40 hover:text-foreground inline-block"
        >
        ...
        {[
          {
            ...
            value: users.filter((u) => u.role !== "DISPATCHER").length,
            icon: Shield,
            color: "text-error",
          },
          {
          ...
          <div
            key={idx}
            className="glass-card p-6 border border-foreground/5 relative overflow-hidden group bg-foreground/5 rounded-3xl"
          >
            ...
            <p className={cn("text-3xl font-black", stat.color)}>
              {stat.value}
            </p>
          </div>
        ))}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead className="bg-foreground/[0.03] border-b border-foreground/5">
              <tr>
                <th className="py-4 px-8 text-[10px] font-black uppercase tracking-widest text-foreground/40">
                  {t("identity")}
                </th>
                <th className="py-4 px-8 text-[10px] font-black uppercase tracking-widest text-foreground/40">
                  {t("accessLevel")}
                ...
                <th className="py-4 px-8 text-[10px] font-black uppercase tracking-widest text-foreground/40 text-right">
                  {t("actions")}
                </th>
              </tr>
            </thead>
            <tbody>
              {users.length > 0 ? (
                ...
                <tr>
                  <td
                    colSpan={4}
                    className="py-10 text-center text-foreground/40 font-medium italic"
                  >
                    {t("noUsers")}
                  </td>
                </tr>
            ...
            >
              <div className="space-y-2">
                ...
                <input
                  name="username"
                  placeholder="op_name"
                  className="w-full bg-foreground/5 border border-foreground/10 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary/50 text-foreground"
                  required
                />
              </div>
              <div className="space-y-2">
              ...
              <button
                type="submit"
                className="w-full bg-primary hover:bg-primary/90 text-primary-foreground font-bold py-4 rounded-xl shadow-lg shadow-primary/20 transition-all active:scale-95 mt-4"
              >
                {t("create")}
              </button>
            </form>
